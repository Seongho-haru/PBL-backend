...
- 채택된 답변이 있는 질문은 미해결(`UNRESOLVED`) 상태로 변경할 수 없습니다.
- 채택된 답변이 있는 경우, 먼저 답변 채택을 취소해야 미해결 상태로 변경할 수 있습니다.
...
- **단, 채택된 답변이 있으면 미해결 상태로 변경할 수 없음**

#### 에러 응답

- **400 Bad Request**: 채택된 답변이 있어 미해결 상태로 변경할 수 없음
  ```json
  {
    "error": "채택된 답변이 있어 질문을 미해결 상태로 변경할 수 없습니다. 먼저 답변 채택을 취소해주세요."
  }
  ```
