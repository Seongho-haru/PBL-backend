...
import com.PBL.lab.judge0.job.ExecutionJob;
 ...
 * 실행 작업 스케줄러 서비스
 * 
 * 이 서비스는 Judge0 시스템에서 코드 실행을 위한 백그라운드 작업들을 관리합니다.
 * JobRunr 라이브러리를 사용하여 안정적이고 신뢰할 수 있는 작업 처리를 제공합니다.
 * 
 * 주요 기능:
 * - 즉시 실행 작업 스케줄링 (executeAsync 호출 시)
 * - 지연 실행 작업 스케줄링 (특정 시간 후 실행)
 * - 실행 중인 작업 취소 및 상태 추적
 * - 작업 큐 통계 정보 제공
 * - 완료된 작업 정리 및 메모리 관리
 * 
...
@Slf4j
public class ExecutionJobScheduler {

    private final JobScheduler jobScheduler;  // JobRunr의 작업 스케줄러 (작업 등록/관리)
    private final ExecutionJob executionJob;  // 실제 코드 실행을 담당하는 작업 클래스
    
    // 스케줄된 작업들을 추적하는 맵 (submission token -> JobId)
    ...
    /**
     * 제출된 코드의 즉시 실행을 스케줄링하는 메서드
     * 
     * 이 메서드는 ExecutionService.executeAsync()에서 호출되어
     * 코드 실행 작업을 백그라운드 큐에 등록합니다.
     * 
     * 처리 과정:
     * 1. JobRunr의 enqueue()를 사용하여 즉시 실행 작업 등록
     * 2. ExecutionJob.executeSubmission()이 백그라운드에서 실행됨
     ...
     */
    public void scheduleExecution(String submissionToken) {
        try {
            log.info("코드 실행 작업 스케줄링 시작 - submission token: {}", submissionToken);
            
            // JobRunr를 통해 ExecutionJob.executeSubmission()을 백그라운드 작업으로 등록
            // enqueue()는 작업을 즉시 실행 큐에 추가하여 가능한 빨리 처리하도록 함
            JobId jobId = jobScheduler.enqueue(() -> executionJob.executeSubmission(submissionToken));
            
            // 작업 ID를 내부 맵에 저장하여 나중에 취소하거나 상태를 확인할 수 있도록 함
            scheduledJobs.put(submissionToken, jobId);
            
            log.debug("작업 스케줄링 완료 - JobId: {}, submission token: {}", jobId, submissionToken);
            
        } catch (Exception e) {
            log.error("작업 스케줄링 실패 - submission token: {}", submissionToken, e);
            throw new RuntimeException("Job scheduling failed", e);
        }
    }

    /**
     * 제출된 코드의 지연 실행을 스케줄링하는 메서드
     * 
     * 이 메서드는 특정 시간 후에 코드 실행을 시작하도록 작업을 예약합니다.
     * 현재는 사용되지 않지만, 향후 기능 확장을 위해 구현되어 있습니다.
     * 
     * 사용 시나리오:
     * - 시스템 부하가 높을 때 실행을 지연시키고 싶은 경우
     * - 특정 시간대에만 코드 실행을 허용하고 싶은 경우
     * - 배치 처리나 스케줄링이 필요한 경우
     * 
     * 처리 과정:
     ...
     * @throws RuntimeException 작업 스케줄링 실패 시 발생
     */
    public void scheduleDelayedExecution(String submissionToken, Duration delay) {
        try {
            log.info("지연 실행 작업 스케줄링 시작 - submission token: {}, 지연 시간: {}", submissionToken, delay);
            ...
            JobId jobId = jobScheduler.schedule(LocalDateTime.now().plus(delay), 
                    () -> executionJob.executeSubmission(submissionToken));
            
            // 작업 ID를 내부 맵에 저장하여 추적 가능하도록 함
            scheduledJobs.put(submissionToken, jobId);
            
            log.debug("지연 실행 작업 스케줄링 완료 - JobId: {}, submission token: {}", jobId, submissionToken);
            
        } catch (Exception e) {
            log.error("지연 실행 작업 스케줄링 실패 - submission token: {}", submissionToken, e);
            throw new RuntimeException("Job scheduling failed", e);
        }
    }

    /**
     * 제출된 코드의 실행 작업을 취소하는 메서드
     * 
     ...
     * 
     * 처리 과정:
     * 1. 내부 맵에서 해당 submission token의 JobId 조회
     * 2. JobRunr의 delete()를 사용하여 작업 취소
     * 3. 내부 맵에서 해당 항목 제거
     * 4. 취소 성공/실패 결과 반환
     * 
     * @param submissionToken 취소할 제출의 고유 토큰
     * @return boolean 취소 성공 시 true, 실패 시 false
     */
    public boolean cancelExecution(String submissionToken) {
        try {
            // 내부 맵에서 해당 submission token의 JobId를 조회하고 제거
            JobId jobId = scheduledJobs.remove(submissionToken);
            
            if (jobId != null) {
                // JobRunr를 통해 실제 작업 취소
                jobScheduler.delete(jobId);
                log.info("실행 작업 취소 완료 - submission token: {}", submissionToken);
                return true;
            } else {
                // 해당 submission token에 대한 스케줄된 작업이 없는 경우
                log.warn("취소할 스케줄된 작업을 찾을 수 없음 - submission token: {}", submissionToken);
                return false;
            }
        } catch (Exception e) {
            log.error("실행 작업 취소 실패 - submission token: {}", submissionToken, e);
            return false;
        }
    }

    /**
     * 제출된 코드의 작업 상태를 조회하는 메서드
     * 
     * 이 메서드는 특정 submission token에 대한 작업의 현재 상태를 반환합니다.
     ...
     * - NOT_FOUND: 해당 submission token에 대한 작업이 없음
     ...
     * 
     * @param submissionToken 상태를 조회할 제출의 고유 토큰
     * @return JobStatus 작업의 현재 상태
     */
    public JobStatus getJobStatus(String submissionToken) {
        // 내부 맵에서 해당 submission token의 JobId 조회
        JobId jobId = scheduledJobs.get(submissionToken);
        ...
        try {
            ...
            log.error("작업 상태 조회 실패 - submission token: {}", submissionToken, e);
    ...
    public void cleanupCompletedJobs() {
        ...
        scheduledJobs.entrySet().removeIf(entry -> {
            try {
                ...
                log.warn("작업 상태 확인 중 오류 발생 - submission token: {}", entry.getKey(), e);
